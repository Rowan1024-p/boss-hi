...
// 🔥🔥🔥 [防重复注入] 检查是否已加载 🔥🔥🔥
if (window.GoodHR_Boss_Loaded) {
    console.log('[Boss.js] 已加载,跳过重复注入');
    // 直接退出,不执行后续代码
} else {
    // 标记为已加载
    window.GoodHR_Boss_Loaded = true;
...
{ 
    ...
    "matched_position": "岗位名(运营主管/供应链/产品开发/无)", 
    "reason": "评价(指出扣分点和加分点,30字内)", 
    "action": "PASS" // 只有完全合适才输出 PASS，否则一律输出 REJECT
}`;

const SUPER_PROMPT = `
你是一名资深跨境电商招聘总监。请先阅读候选人简历，判断其最适合以下哪一个岗位，然后严格按照该岗位的【评分表】进行计算。

【岗位 A：亚马逊运营主管 (Amazon Operation Supervisor)】
总分 100 分。
1. 硬性门槛 (20分):
...
class BossParser extends window.BaseParser {
    ...
    findElements() {
     ...
     return Array.from(items).filter(el => {
      // ✅ 修复:只做基础过滤,不过滤 data-goodhr-done
      // 查重工作由 processedNames (内存记忆) 来做
    ...
    async filterCandidateAsync(element) {
     ...
     try {
      ...
      const aiResult = await this.callAI(detailText.substring(0, 15000));
      ...
      if (isPassingScore && isActionPass && isCleanReason) {
       ...
       this.addStatusLabel(element, `✅ [${aiResult.matched_position}] ${aiResult.score}分: ${aiResult.reason}`, color);
    ...
    async callAI(text) {
     return new Promise((resolve) => {
      chrome.runtime.sendMessage({
       ...
       data: { systemPrompt: SUPER_PROMPT, userContent: `候选人简历内容：\n${text}` }
      }, (response) => {
       if (!response || response.status !== 'success') {
        resolve({ score: 0, reason: "AI未响应", matched_position: "无", action: "REJECT" });
       } else {
